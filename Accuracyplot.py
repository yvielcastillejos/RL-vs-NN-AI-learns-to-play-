...
#	holdon
	...
	plt.plot(x5, validAccRec)
	plt.xlabel(f"step ({100} samples per step)")
	plt.ylabel("Training Accuracy on one batch at a time")
	plt.title(f"Training Accuracy vs Mini-Batches (with Learning rate: {0.1} and batch size:{ 0.1} epoch: {7})")
